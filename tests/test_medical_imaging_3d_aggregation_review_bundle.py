...
ROOT = REPO_ROOT / "src" / "sciona" / "atoms" / "medical_imaging_3d" / "aggregation"
BUNDLE = ROOT / "review_bundle.json"

AGGREGATION_FQDNS = {
    "sciona.atoms.medical_imaging_3d.aggregation.case_probability_from_nodule_scores",
}
...
def test_medical_imaging_3d_aggregation_review_bundle_is_catalog_ready() -> None:
    bundle = _bundle()

    assert bundle["schema_version"] == "1.0"
    assert bundle["bundle_id"] == "sciona.atoms.review_bundle.medical_imaging_3d.aggregation.v1"
    assert bundle["provider_repo"] == "sciona-atoms-bio"
    assert bundle["family"] == "medical_imaging_3d.aggregation"
...
def test_medical_imaging_3d_aggregation_review_bundle_points_to_live_function() -> None:
    rows = {row["atom_name"]: row for row in _bundle()["rows"]}
    assert set(rows) == AGGREGATION_FQDNS
