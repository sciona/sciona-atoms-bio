...
def test_molecular_docking_review_bundle_covers_all_rows() -> None:
    ...
    assert bundle["limitations"] == [
        "Some subfamilies still lack uncertainty evidence: build_interaction_graph, greedy_mapping_d12, quantum_solver, quantum_solver_d12."
    ]
    assert bundle["required_actions"] == [
        "Backfill uncertainty evidence for: build_interaction_graph, greedy_mapping_d12, quantum_solver, quantum_solver_d12."
    ]
    assert len(bundle["rows"]) == len(expected_keys)
    assert {row["atom_fqdn"] for row in bundle["rows"]} == expected_keys
    assert ready_subfamilies == {"greedy_mapping", "minimize_bandwidth", "mwis_sa"}
