...
def test_medical_imaging_3d_preprocessing_review_bundle_is_catalog_ready() -> None:
    bundle = _bundle()

    assert bundle["schema_version"] == "1.0"
    assert bundle["bundle_id"] == "sciona.atoms.review_bundle.medical_imaging_3d.preprocessing.v1"
    assert bundle["provider_repo"] == "sciona-atoms-bio"
    assert bundle["family"] == "medical_imaging_3d.preprocessing"
...
def test_medical_imaging_3d_preprocessing_review_bundle_points_to_live_functions() -> None:
    rows = {row["atom_name"]: row for row in _bundle()["rows"]}
    assert set(rows) == PREPROCESSING_FQDNS
